/** Post-stay thank-you emails (Resend). Mirrors src/lib/postStayThankYouTemplate.ts */
...
function buildPostStayThankYouEmailHtml(payload) {
  ...
  const logoUrl = escapeHtml(resolveLogoUrl(payload.logoUrl));
...
<body style="margin:0;padding:0;background-color:#EAF2F2;font-family:Georgia,'Times New Roman',serif;">
  <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="background-color:#EAF2F2;padding:32px 16px;">
    <tr>
      <td align="center">
        <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="max-width:560px;background-color:#ffffff;border-radius:20px;overflow:hidden;border:1px solid rgba(11,79,92,0.08);">
          <tr>
            <td align="center" style="padding:26px 32px 20px;">
              <img src="${logoUrl}" alt="Vailo" width="140" style="display:block;border:0;height:auto;max-width:140px;margin:0 auto;" />
            </td>
          </tr>
          <tr>
            <td style="background:linear-gradient(135deg,#0B4F5C 0%,#083A43 100%);padding:24px 32px;">
              <p style="margin:0 0 8px;font-family:system-ui,sans-serif;font-size:11px;font-weight:700;letter-spacing:0.18em;text-transform:uppercase;color:rgba(197,160,89,0.95);">Thank you</p>
              <h1 style="margin:0;font-size:26px;line-height:1.25;font-weight:500;color:#ffffff;">We hope you enjoyed your stay</h1>
            </td>
          </tr>
          <tr>
            <td style="padding:32px;font-family:system-ui,sans-serif;color:#051F26;">
              ...
              <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="margin:0 0 24px;background-color:#F8FAFA;border:1px solid #E2E8F0;border-radius:14px;">
                <tr>
                  <td style="padding:18px 20px;">
                    ${property ? `<p style="margin:0 0 8px;font-size:13px;color:#64748B;"><span style="font-weight:700;color:#0B4F5C;">Property</span> ${property}</p>` : ""}
                    ${unit ? `<p style="margin:0 0 8px;font-size:13px;color:#64748B;"><span style="font-weight:700;color:#0B4F5C;">Listing</span> ${unit}</p>` : ""}
                    ${stay ? `<p style="margin:0;font-size:13px;color:#64748B;"><span style="font-weight:700;color:#0B4F5C;">Dates</span> ${stay}</p>` : ""}
                  </td>
                </tr>
              </table>
              ...
              <p style="margin:0;font-size:15px;line-height:1.6;">Warm regards,<br /><span style="color:#0B4F5C;font-weight:600;">${host}</span></p>
            </td>
          </tr>
          <tr>
            <td style="padding:20px 32px 28px;border-top:1px solid #EEF2F4;font-family:system-ui,sans-serif;">
              <p style="margin:0;font-size:11px;color:#94A3B8;">Powered by <a href="https://vailo.app" style="color:#0B4F5C;text-decoration:none;font-weight:600;">Vailo</a></p>
            </td>
          </tr>
        </table>
      </td>
...
async function deliverPostStayThankYouEmail(apiKey, toEmail, payload) {
  ...
  const response = await fetch("https://api.resend.com/emails", {
    ...
    body: JSON.stringify({
      ...
      to: [to],
      subject: buildPostStayThankYouSubject(payload),
      text: buildPostStayThankYouText(payload),
      html: buildPostStayThankYouEmailHtml(payload),
    }),
  });
...
module.exports = {
  buildPostStayThankYouSubject,
  deliverPostStayThankYouEmail,
};
