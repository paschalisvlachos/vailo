/** ISO 3166 country names (English) — bundled so admin dropdowns work without external APIs. */
...
/** Match legacy restcountries `name.common` labels stored in Firestore. */
const FRIENDLY_COUNTRY_LABELS: Record<string, string> = {
...
export function loadCountryNames(): Promise<string[]> {
  ...
  for (const raw of COUNTRY_NAMES) {
    const label = FRIENDLY_COUNTRY_LABELS[raw] ?? raw;
