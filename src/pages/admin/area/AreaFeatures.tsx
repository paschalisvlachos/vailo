        ...
        <div className="flex items-center">
          ...
          <div>
            ...
            <p className="text-gray-500 mt-1">
              Global features & services for <span className="font-bold text-indigo-700">{decodedArea}, {decodedCountry}</span>
            </p>
          </div>
        </div>
        {!isFormOpen && (
          <button onClick={() => { setFormData(initialFormState); setImagePreview(null); setImageFile(null); setIsFormOpen(true); }} className="flex items-center px-4 py-2 bg-indigo-600 text-white text-sm font-bold rounded-lg hover:bg-indigo-700 transition-colors shadow-sm">
      ...
      {isFormOpen && (
        <div className="bg-white rounded-xl shadow-lg border border-gray-200 overflow-hidden mb-8 animate-in fade-in slide-in-from-top-4 duration-300">
          <div className="px-6 py-4 bg-indigo-50 border-b border-indigo-100 flex items-center justify-between">
            ...
            <button type="button" onClick={() => setIsFormOpen(false)} className="text-indigo-500 hover:text-indigo-700 font-medium text-sm">Cancel</button>
          </div>
          ...
          <div className="p-6">
            ...
            <form onSubmit={handleSubmit} className="space-y-8">
              ...
              <div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                  <div>
                    ...
                    <input type="text" name="name" required value={formData.name} onChange={handleChange} className="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none" />
                  </div>
                  <div>
                    <label className="block text-sm font-bold text-gray-700 mb-1">Agreement *</label>
                    <div className="relative">
                      <input type="number" step="0.1" name="agreement" required value={formData.agreement} onChange={handleChange} placeholder="0" className="w-full pl-4 pr-10 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none" />
              ...
              <div>
                ...
                <textarea name="description" required rows={3} value={formData.description} onChange={handleChange} className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none resize-y" />
              </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input type="tel" name="phoneNumber" value={formData.phoneNumber} onChange={handleChange} className="w-full pl-10 pr-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none" />
                    </div>
                  </div>
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input type="tel" name="whatsapp" value={formData.whatsapp} onChange={handleChange} className="w-full pl-10 pr-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none" />
                    </div>
                  </div>
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input type="email" name="email" value={formData.email} onChange={handleChange} className="w-full pl-10 pr-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none" />
                    </div>
                  </div>
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input type="url" name="website" value={formData.website} onChange={handleChange} className="w-full pl-10 pr-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none" />
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="flex flex-col md:flex-row items-start gap-6 bg-indigo-50/50 p-6 rounded-xl border border-indigo-100">
                  ...
                  <div className="flex-1 w-full">
                    ...
                    <input type="file" accept="image/*" onChange={(e) => { if (e.target.files?.[0]) { setImageFile(e.target.files[0]); setImagePreview(URL.createObjectURL(e.target.files[0])); setFormData({...formData, photoUrl: ''}); } }} className="block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-bold file:bg-indigo-50 file:text-indigo-700 hover:file:bg-indigo-100 cursor-pointer mb-4" />
                    
                    {availableMasterPhotos.length > 0 && (
                      <div className="border-t border-indigo-200 pt-4">
                        <p className="text-xs font-bold text-indigo-900 mb-2 uppercase tracking-wider">Or select a Global Category Photo</p>
                ...
                {formData.isLocal && (
                  <div className="bg-purple-50 p-5 rounded-xl border border-purple-100 animate-in fade-in slide-in-from-top-2">
                    ...
                    {aiCategories.length === 0 ? (
                      <p className="text-sm text-purple-600 italic">No AI Categories found. Please add some in the Area Functionality hub.</p>
                    ) : (
              ...
              <div className="flex justify-end pt-4 border-t border-gray-100">
                ...
                <button type="submit" disabled={isSubmitting || isUploadingImage} className="flex items-center px-8 py-3 text-sm font-bold text-white bg-indigo-600 hover:bg-indigo-700 rounded-xl disabled:opacity-50 transition-colors shadow-md hover:shadow-lg">
            ...
            <div key={feat.id} className="bg-white rounded-xl border border-gray-200 overflow-hidden shadow-sm hover:shadow-md transition-shadow flex flex-col">
              <div className="h-48 bg-gray-200 relative">
                ...
                {feat.agreement && feat.agreement !== "0" && (
                  <div className="absolute top-3 right-3 bg-indigo-600 text-white px-2.5 py-1 rounded-full text-xs font-bold shadow-sm">
                    {feat.agreement}%
                  </div>
                )}
              </div>
              <div className="p-5 flex-1 flex flex-col">
                ...
                <div className="mt-auto flex justify-end gap-2 pt-4 border-t border-gray-100">
                  <button onClick={() => { setFormData(feat); setEditingFeatureId(feat.id); setImagePreview(feat.photoUrl); setIsFormOpen(true); }} className="p-2 text-blue-600 hover:bg-blue-50 rounded-lg transition-colors"><Pencil size={18} /></button>
