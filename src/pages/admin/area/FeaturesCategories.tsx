...
export default function FeaturesCategories() {
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    try {
      await addDoc(collection(db, 'countries', decodedCountry, 'areas', areaId, 'featuresCategories'), {
        name: newName.trim(),
      ...
      <div className="flex items-center mb-8">
        ...
        <div>
          <h2 className="text-2xl font-bold text-gray-900 flex items-center">
            <Layers className="mr-3 text-emerald-600" size={28} />
            Features Categories
          </h2>
          <p className="text-gray-500 mt-1">
            Managing categories for <span className="font-bold text-emerald-700">{decodedArea}, {decodedCountry}</span>
          </p>
        </div>
      </div>
        ...
        <div className="md:col-span-2">
          <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
            <div className="px-6 py-4 border-b border-gray-100 bg-gray-50 flex items-center justify-between">
              ...
              <span className="bg-emerald-100 text-emerald-800 text-xs font-bold px-2.5 py-0.5 rounded-full">{categories.length} Total</span>
            </div>
