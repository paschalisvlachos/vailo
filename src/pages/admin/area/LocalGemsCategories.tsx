...
export default function LocalGemsCategories() {
  ...
  useEffect(() => {
    ...
    const colRef = collection(db, 'countries', decodedCountry, 'areas', areaId, 'localGemsCategories');
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    try {
      await addDoc(collection(db, 'countries', decodedCountry, 'areas', areaId, 'localGemsCategories'), {
        name: newName.trim(),
  ...
  const handleSaveEdit = async (id: string, oldName: string) => {
    ...
    try {
      const updatedCount = await renameLocalGemsCategory(
        decodedCountry,
        areaId,
        id,
        oldName,
      ...
      toast.success(
        updatedCount > 0
          ? `Category renamed and updated on ${updatedCount} linked place${updatedCount === 1 ? '' : 's'}.`
          : 'Category renamed.'
      );
    } catch (error) {
                    ...
                    {editingId === cat.id ? (
                      <>
                        <input
                          ...
                          className="flex-1 px-3 py-2 border border-vailo-teal/30 rounded-lg admin-input outline-none text-sm"
                          autoFocus
                          onKeyDown={(e) => {
                      ...
                      <>
                        <span className="font-medium text-gray-900 flex items-center min-w-0">
                          <Tag size={16} className="text-vailo-teal/50 mr-3 shrink-0" />
                        ...
                        <div className="flex items-center gap-1 shrink-0">
                          <button
                            ...
                            className="p-2 text-gray-400 hover:text-vailo-teal hover:bg-vailo-teal/5 rounded-lg transition-colors"
                            title="Edit category"
                          >
                            <Pencil size={18} />
                          </button>
