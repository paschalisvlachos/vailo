...
export default function AiGapAnalyzer() {
  ...
  const analyzeGaps = async () => {
    ...
    try {
      ...
      const featureNames =
        features.length > 0 ? features.map((f) => f.name).join(', ') : 'None added yet';
      ...
      console.error(error);
      setMessages([
        { role: 'ai', text: 'Failed to analyze gaps. Ensure your Firebase AI is configured.' },
      ]);
    } finally {
  ...
  const handleSend = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      const history = messages
        .map((m) => `${m.role === 'ai' ? 'Consultant' : 'Host'}: ${m.text}`)
        .join('\n\n');
      ...
      <div className="flex justify-between items-center mb-6 shrink-0">
        <div>
          <h2 className="text-xl font-bold text-gray-900 flex items-center">
            <Sparkles className="mr-2 text-vailo-teal" size={24} />
            AI Gap Analyzer
          </h2>
          <p className="text-gray-500 text-sm mt-1">
            Consult with Vailo AI to find missing features and opportunities.
          </p>
        </div>
        <button
          onClick={analyzeGaps}
          disabled={isGenerating}
          className="flex items-center px-4 py-2 bg-gradient-to-r from-blue-600 to-indigo-600 text-white font-bold rounded-lg hover:opacity-90 shadow-sm disabled:opacity-50 transition-all text-sm"
        >
          {isGenerating ? (
